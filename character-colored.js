...
const colorBackground = "rgba(0,0,0,.8)";
...
const changeDirectionProbability = 0.5;
const maxPossibleInkUnitsPerSegment = (horizontalCellDotCount * horizontalCellDotCount) * 4;
const linearAnimationIncrement = 0.9;
const lerpAnimationIncrement = 0.2;
const useLerpAnimation = true;
...
const drawAnimatedBrushes = (x, y) => {
  ...
  for (let brush of characterBrushes) {
    // fill(colorBrush);
    fill(hslToHexString(brush.hue++, 100, 80));
    ...
    if (useLerpAnimation) {
      // Lerping
      brush.pixelCoordinateX = lerp(brush.pixelCoordinateX, targetX, lerpAnimationIncrement);
      brush.pixelCoordinateY = lerp(brush.pixelCoordinateY, targetY, lerpAnimationIncrement);
    } else {
      // Linear
      if (brush.pixelCoordinateX < targetX) {
        brush.pixelCoordinateX += linearAnimationIncrement;
      } else if (brush.pixelCoordinateX > targetX) {
        brush.pixelCoordinateX -= linearAnimationIncrement;
      }
      if (brush.pixelCoordinateY < targetY) {
        brush.pixelCoordinateY += linearAnimationIncrement;
      } else if (brush.pixelCoordinateY > targetY) {
        brush.pixelCoordinateY -= linearAnimationIncrement;
      }
    }
    ...
    // move to next vertex
    if (Math.abs(targetX - brush.pixelCoordinateX) < (useLerpAnimation ? lerpAnimationIncrement : (linearAnimationIncrement)) &&
      Math.abs(targetY - brush.pixelCoordinateY) < (useLerpAnimation ? lerpAnimationIncrement : (linearAnimationIncrement))) {

      brush.vertexIndex += brush.direction;
